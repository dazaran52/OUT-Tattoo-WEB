...
# --- Manual Payment Requests (Revolut) ---
...
async def create_payment_request(
    ...
    active = supabase.table("payment_requests").select("*").eq("user_id", current_user.user_id).in_("status", ["pending", "screenshot_uploaded"]).execute()
    if active.data and len(active.data) > 0:
        raise HTTPException(status_code=400, detail="You already have an active payment request.")
...
async def get_active_payment_request(
    current_user: AuthUser = Depends(get_current_user)
):
    supabase = get_supabase_client()
    # Return any pending or screenshot_uploaded request, or recently rejected/approved if we want to notify
    # For now, let's just return pending or screenshot_uploaded or rejected (to show the rejection reason once)
    active = supabase.table("payment_requests") \
        .select("*") \
        .eq("user_id", current_user.user_id) \
        .in_("status", ["pending", "screenshot_uploaded", "rejected"]) \
...
@router.post("/requests/{request_id}/screenshot")
async def upload_screenshot(
    request_id: str,
    req: ScreenshotUpdate,
    current_user: AuthUser = Depends(get_current_user)
):
    supabase = get_supabase_client()
    
    # verify ownership
    pr = supabase.table("payment_requests").select("*").eq("id", request_id).eq("user_id", current_user.user_id).execute()
    if not pr.data:
        raise HTTPException(status_code=404, detail="Request not found")
        
    res = supabase.table("payment_requests").update({
